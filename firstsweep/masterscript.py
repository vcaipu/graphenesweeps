...
def run_qe(inputfile,outputfile):
    ...
    cmd = f"mpirun --oversubscribe -np 16 pw.x -nk 16 -input {inputfile} > {outputfile}"
...
parser.add_argument("--strain", type=float, help="The value of the strain")
parser.add_argument("--guess", type=float, help="The strain of the initial guess")

strain = parser.parse_args().strain
strainString = f"{strain:.1f}"
guess = parser.parse_args().guess

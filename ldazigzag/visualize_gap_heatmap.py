...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="QE run prefix (example: graphene25.1zoom)",
    )
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    parser.add_argument(
        "--show-min",
        action="store_true",
        help="Mark and annotate the minimum direct gap point.",
    )
...
def choose_nocc(root, eigenvalues_ev, nocc_cli=None, fermi_cli_ev=None):
    ...
    if nocc is None or nocc <= 0 or nocc >= nbnd:
        raise ValueError(
            "Could not determine occupied-band count. "
            "Pass --nocc explicitly (must satisfy 1 <= nocc < nbnd)."
        )
...
def build_regular_grid(x, y, z):
    # Round coordinates to avoid tiny float representation mismatch.
    ...
    for xi, yi, zi in zip(x_rounded, y_rounded, z):
        ...
        if not np.isnan(grid[ix, iy]):
            # Duplicate k-point location; not a regular rectangular mesh.
            return None
        grid[ix, iy] = zi
...
def main():
    ...
    min_gap = float(direct_gap_ev[min_idx])
    min_x = float(x[min_idx])
    min_y = float(y[min_idx])

    if args.show_min:
        ax.scatter([min_x], [min_y], s=40, color="black", marker="x", linewidths=1.5)
        ax.annotate(
            f"min gap = {min_gap:.6f} eV",
            (min_x, min_y),
            xytext=(8, 8),
    ...
    outname = f"{args.prefix}_gap_heatmap_{args.axes}.png"
    plt.tight_layout()
    plt.savefig(outname, dpi=300, bbox_inches="tight")
    print(f"Success: wrote heatmap to {outname}")
    print(f"Info: nk={len(direct_gap_ev)}, nbnd={eigenvalues_ev.shape[1]}, min_gap={min_gap:.8f} eV")
